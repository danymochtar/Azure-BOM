...
def build_bcdr_bom(
    ...
    # --- Azure Site Recovery (per protected instance) ---
    ...
    if asr and vm_count > 0:
        lines.append(
            BomLine(
                category="BCDR",
                resource=f"Azure Site Recovery — {vm_count} protected instance(s)",
    ...
    if backup_gb > 0:
        ...
        if chosen:
            lines.append(
                BomLine(
                    category="BCDR",
                    resource=f"Backup Storage ({storage_redundancy}, ~{backup_gb} GB)",
